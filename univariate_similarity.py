...
def analyze(request: AnalysisRequest):
    ...
    similarity_distance = compute_distance(sample_data, [request.target_date_start, request.target_date_end], [request.compare_date_start, request.compare_date_end], user_target_distance)
    ...
    chart_data = {
        "original": create_figure2(original_data, [request.target_date_start, request.target_date_end], request.selected_data, filtered_dates, 'Original', subtract=False, n_steps=request.n_steps),
        "aligned": create_figure2(original_data, [request.target_date_start, request.target_date_end], request.selected_data, filtered_dates, 'Aligned', subtract=True, n_steps=request.n_steps)
    }
...
test_request_data = AnalysisRequest(
    ...
    target_date_end="2023-11-05",
    compare_date_start="2022-10-01",
    compare_date_end="2023-01-01",
    n_steps=0
)
