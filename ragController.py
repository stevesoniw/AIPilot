...
async def prompt_upload_files(request: Request, files: List[UploadFile] = File(...)):
    ...
    user_id = client_user_id if client_user_id else str(uuid.uuid4())  # 새 사용자 ID 생성 또는 클라이언트 ID 사용

    if user_id not in user_files_data:
        user_files_data[user_id] = {}
        user_files_metadata[user_id] = []

    file_id_counter = len(user_files_data[user_id])
    ...
    for uploaded_file in files:
        ...
        user_files_data[user_id][file_id_counter] = {
...
async def delete_file_data(user_id: str, file_id: int):
